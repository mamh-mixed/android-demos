...
- (void)connectionDidFinishLoading:(VTConnectionRequest *)connection
{
    if (connection.tag == 3)
    {
        ...
        if ([[dic objectForKey:@"svValue"] isEqualToNumber:[NSNumber numberWithInt:0]]) {
            _viewController.label.text=[dic objectForKey:@"userkey"];
            ExpandRequest *eRequest=[[ExpandRequest alloc]init];
            [eRequest connectionNet:_Voice_Path andUserKey:[[dict objectForKey:@"result"] objectForKey:@"userkey"]];
        }
        else{
            _viewController.label.text=@"Error";
        }
    }
}
#pragma mark sha1 加密
