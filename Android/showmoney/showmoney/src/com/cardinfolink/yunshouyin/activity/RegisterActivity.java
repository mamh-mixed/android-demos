...
public class RegisterActivity extends BaseActivity {
   ...
   @SuppressLint("NewApi") private boolean validate(){
	  ...
	  if(email.isEmpty()){
		  mAlert_Dialog.show("邮箱不能为空!",  BitmapFactory.decodeResource(this.getResources(), R.drawable.wrong));
		  return false;
	  }
	  if(!checkEmail(email)){
		  mAlert_Dialog.show("邮箱格式不正确!",  BitmapFactory.decodeResource(this.getResources(), R.drawable.wrong));
		  return false;
	  }
	  ...
	  if(password.length()<6){
		  mAlert_Dialog.show("密码不能小于六位!",  BitmapFactory.decodeResource(this.getResources(), R.drawable.wrong));
		  return false;
	  }
	  if(!password.equals(qr_password)){
		  mAlert_Dialog.show("确认密码不一致!",  BitmapFactory.decodeResource(this.getResources(), R.drawable.wrong));
		  return false;
	  }
