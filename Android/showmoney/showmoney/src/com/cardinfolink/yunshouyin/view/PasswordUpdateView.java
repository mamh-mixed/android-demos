	...
	public PasswordUpdateView(Context context) {
	 ...
	 @SuppressLint("NewApi") private boolean validate(){
		 ...
		 String qrpwd=mQrPwdEdit.getText().toString().replace(" ", "");;
		  ...
		  if(oldpwd.isEmpty()){
			  mBaseActivity.alertShow("原密码不能为空!",  BitmapFactory.decodeResource(this.getResources(), R.drawable.wrong));
			  return false;
		  }
		  
		  if(oldpwd.length()<6){
			  mBaseActivity.alertShow("原密码不能小于六位!",  BitmapFactory.decodeResource(this.getResources(), R.drawable.wrong));
			  return false;
		  }
		  ...
		  if(newpwd.isEmpty()){
			  mBaseActivity.alertShow("新密码不能为空!",  BitmapFactory.decodeResource(this.getResources(), R.drawable.wrong));
			  return false;
		  }
		  
		  if(newpwd.length()<6){
			  mBaseActivity.alertShow("新密码不能小于六位!",  BitmapFactory.decodeResource(this.getResources(), R.drawable.wrong));
			  return false;
		  }
		  
		  if(!qrpwd.equals(newpwd)){
			  mBaseActivity.alertShow("确认密码不一致",  BitmapFactory.decodeResource(this.getResources(), R.drawable.wrong));
			  return false;
		  }
