...
func (u *user) CreateUser(data []byte) (ret *model.ResultBody) {
	...
	userOld, err := mongo.UserColl.FindByUserName(user.UserName)
	if err != nil {
		log.Errorf("根据用户名查询用户失败，%s", err)
		return model.NewResultBody(3, "根据用户名查询用户失败")
	}
	if userOld != nil {
		log.Errorf("用户名已存在,userName=%s", user.UserName)
		return model.NewResultBody(4, "用户名已存在")
	}
	err = mongo.UserColl.Add(user)
	if err != nil {
		log.Errorf("创建用户失败,%s", err)
		return model.NewResultBody(5, "创建用户失败")
	}
	ret = &model.ResultBody{
		Status:  0,
		Message: "创建用户成功",
...
// 分页查找用户
func (u *user) Find(userName, nickName, roleName string, size, page int) (ret *model.ResultBody) {
	log.Debugf("userName is %s; nickName is %s,roleName is %s", userName, nickName, roleName)
	...
	users, total, err := mongo.UserColl.PaginationFind(userName, nickName, roleName, size, page)
	if err != nil {
