...
type PurchaseActCouponsReqBody struct {
	CouponsNo      string `json:"couponsNo"`      // 优麦圈电子券号 50 M 优麦圈电子券号
	OldHostTraceNo string `json:"oldHostTraceNo"` // 原交易后台交易流水号 10 M 原W412交易返回里的hostTraceNo值
...
// PurchaseActCouponsReq W452-刷卡活动券验证
type PurchaseActCouponsReq struct {
	Header PurchaseActCouponsReqHeader `json:"header"`
	Body   PurchaseActCouponsReqBody   `json:"body"`
	SpReq  *model.ScanPayRequest       `json:"-" url:"-" bson:"-"`
}

func (req *PurchaseActCouponsReq) GetT() string {
	return "PurchaseActCoupons"
}
func (req *PurchaseActCouponsReq) GetSpReq() *model.ScanPayRequest {
	return req.SpReq
}

type PurchaseActCouponsRespHeader struct {
	Version       string `json:"version"`       // 报文版本号	15	M	当前版本1.0
	TransDirect   string `json:"transDirect"`   // 交易方向	1	M	A：应答
	TransType     string `json:"transType"`     // 交易类型	8	M	原样返回。本交易固定值W452
	MerchantId    string `json:"merchantId"`    // 商户编号	15	M	由优麦圈后台分配给商户的编号
...
type PurchaseActCouponsRespBody struct {
	...
	TransAmount     int    `json:"transAmount"`     // 交易原始金额10 M 原样返回。交易的原始金额,即抵扣前的原价。单位:分
	PayType         int    `json:"payType"`         // 支付方式 2 M 2:银行卡支付;4:微信支付;5: 支付宝支付
	ActualPayAmount int    `json:"actualPayAmount"` // 实际支付金额 10 M 抵扣之后实际支付的金额。单位:分
	ExpDate         string `json:"expDate"`         // 券有效期	10	C1	券的最后可用日期，格式：yyyyMMdd
}
type PurchaseActCouponsResp struct {
	Header PurchaseActCouponsRespHeader `json:"header"`
	Body   PurchaseActCouponsRespBody   `json:"body"`
}
