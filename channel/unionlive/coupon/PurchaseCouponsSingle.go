...
type PurchaseCouponsSingleReqBody struct {
	...
	Cardbin     string `json:"cardbin"`               // 银行卡cardbin或者用户标识等 30 O 如果券配置了限制cardbin，则传入该值会做相应的限制判断
	TransAmount int64  `json:"transAmount,omitempty"` // 交易原始金额 10 M 交易的原始金额,即抵扣前的原价
	PayType     int    `json:"payType,omitempty"`     // 支付方式 2 M 2:银行卡支付;4:微信支付;5: 支付宝支付
}

// PurchaseCouponsSingleReq W462-电子券验证
type PurchaseCouponsSingleReq struct {
	Header PurchaseCouponsSingleReqHeader `json:"header"`
	Body   PurchaseCouponsSingleReqBody   `json:"body"`
	SpReq  *model.ScanPayRequest          `json:"-" url:"-" bson:"-"`
}

func (req *PurchaseCouponsSingleReq) GetT() string {
	return "PurchaseCouponsSingle"
}
func (req *PurchaseCouponsSingleReq) GetSpReq() *model.ScanPayRequest {
	return req.SpReq
}

type PurchaseCouponsSingleRespHeader struct {
	Version       string `json:"version"`       // 报文版本号	15	M	当前版本1.0
	Transdirect   string `json:"transDirect"`   // 交易方向	1	M	A：应答
	Transtype     string `json:"transType"`     // 交易类型	8	M	原样返回。本交易固定值W462
	Merchantid    string `json:"merchantId"`    // 商户编号	15	M	由优麦圈后台分配给商户的编号
	...
	Returncode    string `json:"returnCode"`    // 返回码	4	M	后台交易返回码。 当返回码 returnCode 为 0000 时普通电子券验证成功
	Returnmessage string `json:"returnMessage"` // 返回码描述	100	M	后台返回码描述
}
type PurchaseCouponsSingleRespBody struct {
	...
	Cardbin       string `json:"cardbin"`         // 银行卡cardbin或者用户标识等 30 O 原样返回
	TransAmount   int64  `json:"transAmount"`     // 交易原始金额 10 M 原样返回 交易的原始金额,即抵扣前的原价
	PayType       int    `json:"payType"`         // 支付方式 2 M 原样返回
	...
	Price         int    `json:"Price,omitempty"` // 实际支付金额
}
type PurchaseCouponsSingleResp struct {
	Header PurchaseCouponsSingleRespHeader `json:"header"`
	Body   PurchaseCouponsSingleRespBody   `json:"body"`
}
