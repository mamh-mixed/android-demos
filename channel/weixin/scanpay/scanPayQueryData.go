...
// ScanPayQueryReqData 请求被扫支付API需要提交的数据
type ScanPayQueryReqData struct {
...
func (d *ScanPayQueryReqData) GenSign() {
	buf := bytes.Buffer{}
...
// ScanPayQueryRespData 被扫支付提交Post数据给到API之后，API会返回XML格式的数据，这个类用来装这些数据
type ScanPayQueryRespData struct {
	XMLName xml.Name `xml:"xml"`
